 ...
 *
 */
public class SecurityFileLocatorTest extends PlexusTestCase
{
	...
	/**
	 * run the test if the security file exist on current work station
	 * if not, then it is no point in running the test
	...
	/**
	 * Test the {@link SecurityFileLocator#getLocation()} method.
	 * The test is successful if it finds the {@code settings-security.xml} in the {@code user.home} directory.
	 * <p/>Note that this test will run if and only if current workstation has activated the security.
	 * <p/>Note again, this class is also tested in an integration test.
	 */
	public void testGetLocation() {
		...
		if (run) {
			try {
				...
				Assert.assertNotNull(getContainer().getComponentDescriptor(FileLocator.class.getName(), "settings-security"));
